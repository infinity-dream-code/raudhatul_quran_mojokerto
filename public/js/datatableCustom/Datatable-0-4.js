    ...
    const buttonConfigMap = {
        ...
        pdf: {
            ...
            customize: function (doc) {
                doc.pageSize = options.pdfPageSize ?? 'A4';
                doc.pageOrientation = options.pdfOrientation ?? 'landscape';
                doc.pageMargins = options.pdfMargins ?? [16, 20, 16, 20];
                doc.defaultStyle = {
                    ...(doc.defaultStyle ?? {}),
                    fontSize: options.pdfFontSize ?? 8,
                    noWrap: false
                };
                doc.styles = {
                    ...(doc.styles ?? {}),
                    tableHeader: {
                        ...
                        fontSize: options.pdfHeaderFontSize ?? 9
                    }
                };

                const table = doc.content.find((item) => item.table);
                if (table?.table?.body?.[0]) {
                    const columnCount = table.table.body[0].length;
                    const providedWidths = Array.isArray(options.pdfWidths) && options.pdfWidths.length === columnCount
                        ? options.pdfWidths
                        : null;
                    table.table.widths = providedWidths ?? Array(columnCount).fill('*');
                    table.layout = {
                        hLineWidth: () => 0.5,
                        vLineWidth: () => 0.5,
                        hLineColor: () => '#d5d5d5',
                    ...
                    table.table.body = table.table.body.map((row) =>
                        row.map((cell) => {
                            if (cell && typeof cell === 'object') {
                                return {...cell, noWrap: false};
                            }
                            return {text: cell ?? '', noWrap: false};
                        })
                    );
                }
        ...
        print: {
            ...
            customize: function (win) {
                if (typeof options.printCustomize === 'function') {
                    options.printCustomize(win);
                }
            },
        },
    };
...
async function dataTableCreate(options) {
    ...
    DT[`${options.tableId}`] = idTable.DataTable({
        ...
        createdRow: function (row, data, dataIndex) {
            ...
            const key = row.id ?? data.id ?? data.uuid ?? (start + dataIndex);

            row.id = `${options.tableId}-row-${key}`;
            row.dataset.key = key;

            // console.log(key, .id)

            // if (!options.rowId){
            //     row.setAttribute('id', `${options.tableId}-row-` + key);
            // }
        ...
        // createdRow: function (row, data, dataIndex) {
        // row.dataset.key = data.id ?? data.uuid ?? dataIndex;
        //
        // if (!options.rowId){
                                ...
                                renderFunc = function (data, type, row, meta) {
                                    if (type === 'display' || type === 'filter') {
                                        if (options.select) {
                                            if (!row._dtKey) {
                                                if (options.rowId) {
                                                    row._dtKey = data[options.rowId];
                                                } else {
                                                    const api = new $.fn.dataTable.Api(meta.settings);
                                                    const start = api.page.info().start;

                                                    row._dtKey = start + meta.row;
                                                }
                                            }

                                            return `<input type="checkbox" id="${options.tableId}-checkbox-${row._dtKey}" class="dt-checkboxes form-check-input" value="${row._dtKey}" aria-selected="false">`;
                                        }
...
function parseArrayForRow(data, currency = false, keyLabel = true) {
    return Object.entries(data)
        .map(([key, value]) => {
            ...
            return keyLabel
                ? `<li><strong>${label}</strong>: ${formattedValue}</li>`
